...
export default function AdminTeamPage() {
  ...
  const [form, setForm] = useState({
    name: "", role: "", description: "", image: "", council: false, object_position: "",
  });

  const members = (data?.data || []) as TeamMember[];

  const resetForm = () => {
    setForm({ name: "", role: "", description: "", image: "", council: false, object_position: "" });
    setEditing(null);
    setShowForm(false);
  };
  ...
  const openEdit = (m: TeamMember) => {
    setForm({
      name: m.name, role: m.role, description: m.description || "", image: m.image || "",
      council: m.council, object_position: m.object_position || "",
    });
    setEditing(m);
    setShowForm(true);
  };
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-[#0A0E27]/80 border border-[#00A3FF]/10 rounded-xl p-4 space-y-3">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <input placeholder="Object Position (e.g. object-top)" value={form.object_position} onChange={(e) => setForm({ ...form, object_position: e.target.value })} className="px-3 py-2 rounded-lg bg-[#050816] border border-[#00A3FF]/20 text-white text-sm" />
          ...
          <textarea placeholder="Description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} className="w-full px-3 py-2 rounded-lg bg-[#050816] border border-[#00A3FF]/20 text-white text-sm" rows={2} />
          <div className="flex items-center gap-3">
            <input type="file" accept="image/*" onChange={handleFileUpload} className="text-sm text-gray-400" />
            {form.image && <span className="text-xs text-gray-500 truncate">{form.image}</span>}
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#00A3FF]/10 text-gray-400">
                  ...
                  <th className="text-left p-3 hidden sm:table-cell">Council</th>
              ...
              <tbody>
                {members.map((m) => (
                  <tr key={m.id} className="border-b border-white/5 text-white hover:bg-white/5">
                    ...
                    <td className="p-3 text-gray-400 hidden sm:table-cell">{m.council ? "Yes" : "No"}</td>
